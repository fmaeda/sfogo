...
function useGlobalStyles(duration: number, hideScrollbars: boolean): any {
  const identifier = React.useMemo(
    () => Math.random().toString(36).substr(2),
    [],
  );
  const classNames = React.useMemo(() => getClassNames(identifier), [
    identifier,
  ]);
  ...
  React.useEffect(() => {
    ...
    return function () {
      const stylesheet = document.querySelector(
        `style[data-react-bottom-drawer='${identifier}']`,
      );
      if (stylesheet) {
        stylesheet.remove();
      }
